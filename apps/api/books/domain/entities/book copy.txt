...
export class BookBase {
  ...
  static create(props: BookBaseProps): Either<BookErrors, BookBase> {
    ...
    const isbnResp = BookValidationService.validateISBN(props.isbn);
    if (isLeft(isbnResp)) return isbnResp;

    const editionResp = BookValidationService.validateEdition(props.edition);
    if (isLeft(editionResp)) return editionResp;

    const titleResp = BookValidationService.validateTitle(props.title);
    if (isLeft(titleResp)) return titleResp;

    const coverResp = BookValidationService.validateCover(props.cover);
    if (isLeft(coverResp)) return coverResp;

    const yearResp = BookValidationService.validateYear(props.year);
    if (isLeft(yearResp)) return yearResp;

    const authorsResp = BookValidationService.validateAuthors(props.authors);
    if (isLeft(authorsResp)) return authorsResp;

    if (props.description !== undefined) {
      const descriptionResp = BookValidationService.validateDescription(props.description);
      if (isLeft(descriptionResp)) return descriptionResp;
    }

    const categoriesResp = BookValidationService.validateCategories(props.categories);
    if (isLeft(categoriesResp)) return categoriesResp;

    const tagsResp = BookValidationService.validateTags(props.tags);
    if (isLeft(tagsResp)) return tagsResp;

    const sourcesResp = BookValidationService.validateSources(props.sources);
